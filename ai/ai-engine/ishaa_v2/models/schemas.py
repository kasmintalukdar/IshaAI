...
# ==============================================================
# FILE: ishaa_v2/models/schemas.py
#
# PURPOSE:
#   All Pydantic v2 data models for the ishaa.ai API.
#   These serve two purposes:
...
# ─────────────────────────────────────────────────────────────
# SECTION 5: ADMIN MODELS
# ─────────────────────────────────────────────────────────────

class AdminSubscriptionUpdate(BaseModel):
    """Request body for PATCH /admin/subscription"""
    user_email: Optional[str] = Field(None, description="Lookup user by email")
    user_id:    Optional[str] = Field(None, description="Lookup user by _id")
    plan:       str = Field(..., description="Target plan: free | pro | premium")
    expires_at: Optional[datetime] = Field(None, description="ISO8601 expiry. null = no expiry")
    reason:     Optional[str] = Field("admin_update", max_length=200)

    @field_validator("plan")
    @classmethod
    def valid_plan(cls, v: str) -> str:
        if v not in {"free", "pro", "premium"}:
            raise ValueError("plan must be 'free', 'pro', or 'premium'")
        return v

    @model_validator(mode="after")
    def check_identifier_present(self):
        if not self.user_email and not self.user_id:
            raise ValueError("Either user_email or user_id must be provided")
        return self


class AdminSubscriptionResponse(BaseModel):
    """Response from PATCH /admin/subscription"""
    status:       str
    user_id:      str
    email:        str
    old_plan:     str
...
class SubscriptionDetailResponse(BaseModel):
    """Response from GET /admin/subscription/{user_id}"""
    user_id:        str
    email:          str
    subscription:   dict
    effective_plan: str
