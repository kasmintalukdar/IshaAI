...
# ==============================================================
# FILE: ishaa_v2/services/ai_service.py
#
# PURPOSE:
#   Orchestrates AI tutoring sessions. Fetches question and user
#   context from MongoDB, builds the system prompt, and exposes
...
_USER_PROJECTION = {
    ...
    "dashboard_insight.status.recent_accuracy": 1,
    "ai_report.predicted_percentile": 1,
    "ai_report.weakness_summary": 1,
    "ai_report.root_cause_analysis": 1,
    "subscription.plan": 1,
}
...
class AIHelpSession:
    ...
    async def check_misconception(self, student_text: str) -> dict:
        ...
        try:
            ...
            logger.warning(f"misconception_check_failed error={str(e)}")
...
async def load_session(
    ...
    try:
        ...
        # Keep string query if user_id is not an ObjectId.
        user_query = {"_id": user_id}
    ...
    q_doc, u_doc = await asyncio.gather(
        questions_col.find_one({"_id": oid}, _QUESTION_PROJECTION),
        users_col.find_one(user_query, _USER_PROJECTION),
    )
    ...
    user_ctx = UserContext(
        ...
        recent_accuracy      = recent_accuracy,
        predicted_percentile = u_doc.get("ai_report", {}).get("predicted_percentile"),
        weakness_summary     = u_doc.get("ai_report", {}).get("weakness_summary"),
        root_cause_analysis  = u_doc.get("ai_report", {}).get("root_cause_analysis"),
    )
